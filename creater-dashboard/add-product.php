...
if (isset($_POST['submit'])) {
    $result = uploadImage('product_image', '../uploads');
   
$product_name = $_POST[ 'product_name' ];
$product_price = $_POST[ 'product_price' ];
$product_description = $_POST[ 'product_description' ];
// $product_image = $imageIsValid = true ? $fullPath: 'img.png';
$product_author = $_SESSION[ 'id' ];
$product_status = 0;
$product_data = $conn->prepare( 'INSERT INTO product_data (product_name, product_price, product_description,
product_image, product_author, product_status) VALUES (?, ?, ?, ?, ?, ?)' );
$product_data->bind_param("sissii", $product_name, $product_price, $product_description, $product_image,
$product_author , $product_status);

if (isset($result['success'])) {
    $upload_message ="Image uploaded successfully: " . $result['file_name'];
    $product_data->execute();
} else {
    $upload_message = "Error: " . $result['error'];
}
}
...
<section class='w-full bg-gray-800 shadow-2xl p-8 w-full mx-auto h-screen'>
    ...
    <form action='' method='POST' class='space-y-6 max-w-[700px] mx-auto' enctype='multipart/form-data'>
        ...
        <div>
            ...
            <p class="text-[red] text-sm mt-1"><?php echo $upload_message ?></p>
        </div>
