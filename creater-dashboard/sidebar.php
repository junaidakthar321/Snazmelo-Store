...
<aside class="w-64 bg-gray-800 shadow-lg fixed inset-y-0 left-0 z-50 flex-col md:flex hidden-mobile">
    ...
    <nav class="flex-grow p-4 space-y-2">
        ...
        <a href='<?php echo  $url."admin/profile.php";?>' class="<?= strpos($current_path, 'admin/profile.php') !== false ? 'bg-gray-700 text-white' : 'text-gray-300 bg-transparent' ?>  flex items-center
            space-x-3 p-3 rounded-lg  hover:bg-gray-700 hover:text-white transition-colors duration-200">
            <i class="fas fa-users w-5 h-5"></i>
            <span>Profile</span>
        </a>
        <a href='<?php echo  $url."creater-dashboard/product.php";?>'
            class='<?= strpos($current_path, 'creater-dashboard/product.php') !== false ? 'bg-gray-700 text-white' : 'text-gray-300 bg-transparent' ?> flex items-center space-x-3 p-3 rounded-lg text-gray-300 hover:bg-gray-700 hover:text-white transition-colors duration-200'>
